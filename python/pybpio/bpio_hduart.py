"""
BPIO Half-Duplex UART Implementation for Bus Pirate 5

This file implements the Half-Duplex UART protocol interface for the Bus Pirate
BPIO2 flatbuffer protocol. It provides both synchronous transactions and
asynchronous data monitoring capabilities.

Half-duplex UART uses a single RXTX pin for both transmit and receive, unlike
full-duplex UART which has separate TX and RX pins. This is useful for protocols
like DMX, LIN bus, or single-wire debug interfaces.

Usage:
    from pybpio.bpio_client import BPIOClient
    from pybpio.bpio_hduart import BPIOHDUART

    # Connect to Bus Pirate
    client = BPIOClient('COM3')

    # Initialize half-duplex UART interface
    hduart = BPIOHDUART(client)
    hduart.configure(speed=115200, parity=False, data_bits=8, stop_bits=1)

    # Send data
    hduart.write(b"Hello World!\r\n")

    # Receive data
    response = hduart.read(10)

    # Monitor for asynchronous data
    async_data = hduart.read_async()
...
class BPIOHDUART(BPIOBase):
    ...
    def configure(self, speed=115200, data_bits=8, parity=False, stop_bits=1,
                  async_callback=None, **kwargs):
        """Configure half-duplex UART mode
        ...
        Args:
            ...
            async_callback (function): Optional callback for async data:
                                      callback(data_bytes)
                                      If provided, async data goes to
                                      callback instead of buffer
        ...
        # Set the half-duplex UART configuration parameters
        ...
        # Automatically start async monitoring when HDUART is configured
        if success:
    ...
    def write(self, data):
        """Write data to half-duplex UART
    ...
    def read(self, num_bytes):
        """Read bytes from half-duplex UART
    ...
    def transfer(self, write_data, read_bytes=None):
        """Perform half-duplex UART write followed by read
        ...
        if not self.configured:
            print("HDUART not configured. Call configure() first.")
            return None
    ...
    def read_async(self, clear_buffer=True):
        ...
        half-duplex UART interface. Async monitoring starts automatically when
        configure() is called, so you can simply call read_async() at any time
        to get data.
    ...
    def _start_async_monitoring(self):
        """Internal: Start monitoring for asynchronous HDUART data"""
        if self._monitoring:
            return
    ...
    def _stop_async_monitoring(self):
        """Internal: Stop monitoring for asynchronous HDUART data"""
        self._monitoring = False
